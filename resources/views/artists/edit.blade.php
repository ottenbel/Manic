...
<div class="container">
	...
	@if(($global_aliases->count() > 0) || (Auth::user()))
		...
		@if(Auth::user())
			<form method="POST" action="/artist_alias/{{$tagObject->id}}" enctype="multipart/form-data">
				{{ csrf_field() }}
		...
		@if($global_aliases->count() > 0)
			<div>
				...
				@if($global_list_order == "asc")
					<b><a href="/artist/{{$tagObject->id}}?global_order=asc">Ascending</a></b> <a href="/artist/{{$tagObject->id}}?global_order=desc">Descending</a>
				@elseif($global_list_order == "desc")
					<a href="/artist/{{$tagObject->id}}?global_order=asc">Ascending</a> <b><a href="/artist/{{$tagObject->id}}?global_order=desc">Descending</a></b>
				@endif
			</div>
		...
		@if($personal_aliases->count() > 0)
			<div>
				...
				@if($personal_list_order == "asc")
					<b><a href="/artist/{{$tagObject->id}}?personal_order=asc">Ascending</a></b> <a href="/artist/{{$tagObject->id}}?personal_order=desc">Descending</a>
				@elseif($personal_list_order == "desc")
					<a href="/artist/{{$tagObject->id}}?personal_order=asc">Ascending</a> <b><a href="/artist/{{$tagObject->id}}?personal_order=desc">Descending</a></b>
				@endif
			</div>
