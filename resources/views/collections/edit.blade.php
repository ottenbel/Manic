...
<div class="container">
	@can('update', $collection)
			...
			@can('delete', $collection)
				<div class="col-xs-4 text-right">
					<form method="POST" action="{{route('delete_collection', ['collection' => $collection])}}">
						...
						{{ Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i> Delete Collection', array('type' => 'submit', 'class' => 'btn btn-danger', 'onclick' =>'ConfirmDelete(event)')) }}
					</form>
				</div>
			@endcan
		</div>
		...
		<form method="POST" action="{{route('update_collection', ['collection' => $collection])}}" enctype="multipart/form-data">
			...
			@include('partials.collection.show', ['volumes' => $collection->volumes, 'editVolume' => true, 'editChapter' => true, 'scanalatorLinkRoute' => 'edit_scanalator', 'hideVolumes' => false, 'isChapter' => false])
